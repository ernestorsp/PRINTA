...
function calculateShippingDate(orderDate,origin){
  const ss=SpreadsheetApp.openById(SPREADSHEET_ID);
  const settings=getSettings_(ss);
  const days=getHandlingDays_(origin,settings);
  return addBusinessDays_(parseDate_(orderDate)||new Date(),days);
}
...
function saveOrder(o){
  ...
  const now=new Date(),id=o.id||nextId_(sh,'PC');
  const orderDate=o.fecha||formatDate_(now);
  const settings=getSettings_(ss);
...
function getSettings_(ss){
  ...
  const out={tiktokDays:2,printaDays:3};
  rows.forEach(r=>{if(r[0]==='TIKTOK_DIAS_ENVIO')out.tiktokDays=number_(r[1],2);if(r[0]==='PRINTA_DIAS_ENVIO')out.printaDays=number_(r[1],3)});
  return out;
}
